...
def prever_vies_fisico(rastreador, table_id):
    """
    Gera uma 'previsão' no mesmo formato usado pelo módulo de ML por dúzia,
    para poder ser plugada no lugar de duzia_ai.prever() quando o modo
    'Viés Físico' estiver selecionado.

    Aposta em NÚMEROS específicos suspeitos (nunca em dúzia — desgaste
    mecânico não respeita fronteira de dúzia). Se não houver números
    suspeitos confirmados, NÃO entra — mesmo que isso signifique nunca
    entrar durante semanas. Isso é o comportamento correto, não uma falha.
    """
    relatorio = rastreador.analisar_mesa(table_id)

    if relatorio["status"] in ("sem_dados", "insuficiente"):
        faltam = relatorio.get("faltam", relatorio.get("n_minimo", 0))
        return {
            "entrar": False,
            "motivo": f"📊 Coletando dados da mesa ({relatorio['n_total']}/{relatorio.get('n_minimo', RastreadorViesFisico.N_MINIMO_TESTE)}, faltam {faltam})",
    ...
    if not relatorio["significativo_global"]:
        return {
            "entrar": False,
            "motivo": f"📊 Mesa estatisticamente uniforme até agora (N={relatorio['n_total']}, χ²={relatorio['chi2']}, p={relatorio['p_global']}) — sem entrada",
            "relatorio_vies_fisico": relatorio,
        }

    suspeitos = relatorio["numeros_suspeitos"]
    if not suspeitos:
        return {
            "entrar": False,
            "motivo": f"📊 Desvio global detectado mas nenhum número passou na correção de Bonferroni (N={relatorio['n_total']})",
            "relatorio_vies_fisico": relatorio,
        }

    numeros_apostar = [s["numero"] for s in suspeitos]
    melhor = suspeitos[0]
    return {
        "entrar": True,
        "motivo": (
            f"🎯 Viés físico — mesa N={relatorio['n_total']} | "
            f"nº {melhor['numero']} saiu {melhor['observado']}x (esperado {melhor['esperado']}) "
            f"| z={melhor['z']} p={melhor['p']} (Bonferroni) | HIPÓTESE, não certeza"
        ),
        "numeros_apostar": numeros_apostar,
        "duzia": None,
        "incluir_zero": 0 in numeros_apostar,
        "confianca": None,  # não existe "confiança calibrada" aqui, de propósito
        "relatorio_vies_fisico": relatorio,
    }


def renderizar_painel_vies_fisico(st, rastreador, table_id, table_name):
    """Painel Streamlit honesto — mostra 'sem dados suficientes' na maior
    parte do tempo, e isso é o esperado."""
    relatorio = rastreador.analisar_mesa(table_id)
    st.markdown(f"### 🎯 Viés Físico — {table_name}")
    ...
    if relatorio["status"] == "insuficiente":
        st.warning(
            ...
            f"(faltam {relatorio['faltam']}). Abaixo disso o teste não tem poder estatístico."
        )
    ...
    st.caption(f"χ² = {relatorio['chi2']} (36 gl) | p global = {relatorio['p_global']}")

    if relatorio["significativo_global"]:
        st.error(f"⚠️ Desvio global significativo (p < {RastreadorViesFisico.ALPHA_GLOBAL})")
        if relatorio["numeros_suspeitos"]:
            st.write("Números com desvio individual significativo (Bonferroni):")
            for s in relatorio["numeros_suspeitos"]:
                st.caption(
                    f"• Nº {s['numero']}: saiu {s['observado']}x (esperado {s['esperado']}) "
                    f"| z={s['z']} | p={s['p']}"
                )
            st.caption("⚠️ Tratar como hipótese a confirmar, não como padrão garantido — "
                       "ver limitação de 'peeking contínuo' na documentação do módulo.")
        else:
            st.caption("Nenhum número individual passou na correção de Bonferroni.")
    else:
        st.success("✅ Mesa estatisticamente uniforme até agora — sem viés detectável")
