...
def salvar_sessao():
    ...
    try:
        ...
        session_data = {
            ...
            'sensibilidade_pingpong': st.session_state.get('sensibilidade_pingpong', 3),
...
# =============================
# [Todas as 35 classes de estratégia permanecem idênticas]
# EstrategiaSniperElite, EstrategiaLuckyVizinhos, EstrategiaRepeticao, etc.
# ... (código das 35 estratégias mantido igual)
...
# =============================
# 🆕 MOTOR DE CONFLUÊNCIA COM SENSIBILIDADE AJUSTÁVEL DO PING-PONG
# =============================

class MotorConfluenciaDuziasColunas:
    """
    Motor de Confluência para Dúzias e Colunas
    Com sensibilidade ajustável do detector de Ping-Pong
    
    sensibilidade_pingpong:
    - 1 = Muito Agressivo (bloqueia com 3 giros sem repetição)
    - 2 = Agressivo (bloqueia com 4 giros sem repetição) [PADRÃO]
    - 3 = Moderado (bloqueia com 5 giros sem repetição)
    - 4 = Conservador (bloqueia com 6 giros sem repetição)
    """
    ...
    def analisar_confluencia(self, historico_numeros, sensibilidade_pingpong=2):
        ...
        # ESTRATÉGIA 4: Detector de Alternância com SENSIBILIDADE AJUSTÁVEL
        # Mapeia sensibilidade para quantidade de giros a verificar
        ...
        coluna_map = {1: self.roleta.coluna1, 2: self.roleta.coluna2, 3: self.roleta.coluna3}
        ...
        if pontos >= 3:
            forca = 80; qualidade = "EXCELENTE"; mensagem = "ENTRADA FORTE"
        elif pontos == 2:
            forca = 65; qualidade = "BOA"; mensagem = "ENTRADA MODERADA"
        elif pontos == 1:
            forca = 50; qualidade = "REGULAR"; mensagem = "ENTRADA FRACA"
        else:
            forca = 30; qualidade = "RUIM"; mensagem = "SEM PADRÃO CLARO"
        ...
        if st.session_state.get('modo_duzias_colunas', False):
            ...
            if confluencia and confluencia['pontos'] >= 1:
                return {
                    ...
                    'estrategias_ativas': [f"Score: {confluencia['pontos']}/{confluencia['pontos_maximos']}"],
...
# APLICAÇÃO STREAMLIT
# =============================
st.set_page_config(page_title="🎯 Roleta Bot Pro v22 - Confluência Avançada", layout="centered")
st.title("🎯 Roleta Bot Pro v22 - Elite Master")
...
with st.sidebar:
    ...
    st.session_state.modo_duzias_colunas = st.checkbox(
        "🎯 ATIVAR MODO CONFLUÊNCIA (Peso Duplo nos Terminais)",
    ...
    if st.session_state.modo_duzias_colunas:
        ...
        st.session_state.sensibilidade_pingpong = st.select_slider(
            "🛡️ Sensibilidade do Ping-Pong",
            options=[1, 2, 3, 4],
            value=st.session_state.sensibilidade_pingpong,
            help="1=Muito Agressivo | 2=Agressivo | 3=Moderado | 4=Conservador"
        )
        mapa_sens = {1: "Muito Agressivo (3g)", 2: "Agressivo (4g)", 3: "Moderado (5g)", 4: "Conservador (6g)"}
        st.caption(f"Detector: {mapa_sens[st.session_state.sensibilidade_pingpong]}")
    else:
        st.info("📊 Modo padrão: 35 estratégias")
    
    st.markdown("---")
    with st.expander("🔄 Inversão Automática", expanded=True):
        st.session_state.modo_inversao_auto = st.checkbox("🔄 ATIVAR INVERSÃO AUTOMÁTICA", value=st.session_state.modo_inversao_auto)
...
if sis.entrada_ativa:
    ...
    cor = "green" if ent['forca_real'] >= 70 else "orange" if ent['forca_real'] >= 55 else "gray"
    ...
    if len(hist_num) >= janela_analise:
        ...
        if validos:
            ...
            with c1:
                col_quente = Counter(colunas).most_common(1)[0][0]
                duz_quente = Counter(duzias).most_common(1)[0][0]
                st.write("**1. Interseção Básica (+1):**")
                ...
                st.write("**2. Força de Terminais (+2):**")
                st.success(f"🎯 Terminais Quentes: {top_terminais}")
                if col_terminal_dominante != 0: st.caption(f"→ +2 pontos Coluna {col_terminal_dominante}")
            ...
            with c2:
                ...
                st.write("**3. Leitura do Cilindro (+1):**")
                ...
                st.write(f"**4. Comportamento ({giros_verificar}g):**")
                if not tem_repeticao and len(ultimas_cols) >= 2:
                    st.error(f"🏓 Ping-Pong! (-1 na Col {colunas[-1]})")
            ...
            if pontos >= 3: st.success(f"### 🎯 ENTRADA FORTE: Coluna {coluna_vencedora} ({pontos}/4 confirmações)")
            elif pontos == 2: st.warning(f"### ⚠️ ENTRADA MODERADA: Coluna {coluna_vencedora} ({pontos}/4 confirmações)")
            elif pontos == 1: st.info(f"### 💡 ENTRADA FRACA: Coluna {coluna_vencedora} ({pontos}/4 confirmações)")
            else: st.error("### 🛑 SEM PADRÃO CLARO: Fique de fora.")
            
            st.caption(f"Placar: Col 1 ({score_colunas[1]}pts) | Col 2 ({score_colunas[2]}pts) | Col 3 ({score_colunas[3]}pts)")
    else:
