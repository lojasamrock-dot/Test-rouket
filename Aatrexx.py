...
class RoletaBase:
    def __init__(self):
        ...
        # 🆕 Mapa de vizinhos próximos (raio 1) para Micro-Clusters
    ...
    def analisar(self, historico):
        ...
        if len(historico) >= 3:
            if historico[-3] == ultimo:
                return {'base': {ultimo, historico[-2]}, 'forca': 60, 'estrategias': [f'Gap1']}
        if len(historico) >= 4:
            if historico[-4] == ultimo:
                return {'base': {ultimo, historico[-2], historico[-3]}, 'forca': 50, 'estrategias': [f'Gap2']}
        if len(historico) >= 5:
            if historico[-5] == ultimo:
                return {'base': {ultimo, historico[-2], historico[-3]}, 'forca': 40, 'estrategias': [f'Gap3']}
...
class EstrategiaCiclosQuadrantes:
    ...
    def analisar(self, historico, janela=8):
        ...
        if len(ausentes) >= 1:
            ...
            return {'base': base, 'forca': min(100, forca), 'estrategias': [f"Quadrantes {ausentes} atrasados"]}
        return None
...
class EstrategiaProtecaoZero:
    ...
    def analisar(self, historico):
        ...
        if zeros_15 >= 2:
            return {'base': self.roleta.vizinhos_zero.copy(), 'forca': 65 + (zeros_15 * 5), 'estrategias': [f"Zero Agressivo"]}
        if zeros_30 >= 2:
            return {'base': self.roleta.vizinhos_zero.copy(), 'forca': 50, 'estrategias': [f"Zero Ativo"]}
...
class EstrategiaLoopTerminal:
    ...
    def analisar(self, historico):
        ...
        if 4 in terminais_quentes:
            base.update(self.terminais[1])
        forca = 30 * len(terminais_quentes)
        return {'base': base, 'forca': min(100, forca), 'estrategias': [f"Loop Terminais: {terminais_quentes}"]}
...
class EstrategiaEspelhoTemporal:
    """
    Detecta números que retornam em ciclos de 3-4 giros.
    Padrão observado: 27 → ... → 27 → ... → 27
                     29 → ... → 29
                     9 → ... → 9 → ... → 9
    """
    def __init__(self):
    ...
    def analisar(self, historico):
        ...
        # Analisa os últimos 20 giros para encontrar ciclos de retorno
        recentes = historico[-20:]
        ...
        # Números que apareceram 3+ vezes com gaps de 3-4
        for num, posicoes in ciclos.items():
            if len(posicoes) >= 3:
                gaps = [posicoes[i+1] - posicoes[i] for i in range(len(posicoes)-1)]
                # Verifica se os gaps são consistentes (3-4 giros)
                gaps_consistentes = [g for g in gaps if 3 <= g <= 4]
                if len(gaps_consistentes) >= 2:
                    base.add(num)
                    forca += 40
                    estrategias.append(f"🔄 Espelho Temporal: {num} (ciclo {gaps})")
        
        # 🆕 Também verifica se o último número está em ciclo
        ultimo = historico[-1]
        if ultimo in ciclos and len(ciclos[ultimo]) >= 2:
            ...
            if ultima_vez is not None:
                ...
                if 3 <= gap_desde_ultima <= 5:
                    base.add(ultimo)
                    forca += 50
                    estrategias.append(f"🎯 Espelho Ativo: {ultimo} (gap {gap_desde_ultima})")
...
class EstrategiaMicroClusters:
    """
    Detecta quando números vizinhos na roda aparecem em sequência.
    Padrão: 7 → 18 (vizinhos), 31 → 16, 10 → 9, 4 → 6
    """
    def __init__(self, roleta):
        self.roleta = roleta
    ...
    def analisar(self, historico):
        ...
        # Verifica se os últimos números formam clusters na roda
        clusters = []
        ...
        if len(clusters) >= 2:
            # Pega os vizinhos do último número
            ultimo = historico[-1]
            vizinhos_ultimo = self.roleta.get_vizinhos(ultimo, 3)
            base.update(vizinhos_ultimo)
            forca += 45 + (len(clusters) * 10)
            estrategias.append(f"🔗 Micro-Cluster: {len(clusters)} pares vizinhos")
        
        # 🆕 Verifica se há padrão de salto entre vizinhos
        if len(historico) >= 4:
            ultimo = historico[-1]
            penultimo = historico[-2]
            if penultimo in self.roleta.vizinhos_proximos.get(ultimo, set()):
                # Se os dois últimos são vizinhos, aposta nos vizinhos do último
                vizinhos = self.roleta.get_vizinhos(ultimo, 2)
...
class EstrategiaRitmoRepeticao:
    """
    Captura padrões de ciclo 3-3-2 ou 2-3-3 na sequência.
    Padrão: 24→15→26→29 (ciclo) depois repete padrão similar
    """
    def __init__(self):
        pass
    ...
    def analisar(self, historico):
        ...
        if len(numeros_repetidos) >= 3:
            ...
            # Números com gap médio entre 2 e 4 (ritmo rápido)
        ...
        if len(historico) >= 6:
            ...
            # Verifica se há padrão A-B-C-A-B (sugere C como próximo)
            if len(set(ultimos_6[:3])) == 3 and ultimos_6[0] == ultimos_6[3] and ultimos_6[1] == ultimos_6[4]:
                ...
                base.update(self._get_vizinhos_simples(proximo_candidato, 2))
                forca += 55
    ...
    def _get_vizinhos_simples(self, numero, raio):
        """Versão simplificada de vizinhos"""
        race = [0, 32, 15, 19, 4, 21, 2, 25, 17, 34, 6, 27, 13, 36, 11, 30, 8, 23, 10, 5, 24, 16, 33, 1, 20, 14, 31, 9, 22, 18, 29, 7, 28, 12, 35, 3, 26]
        if numero not in race:
            return []
        idx = race.index(numero)
        return [race[(idx + i) % 37] for i in range(-raio, raio + 1)]
...
class EstrategiaTerminal369:
    """
    Especializada nos loops dos terminais 3, 6 e 9.
    Padrão: 13,33,3,13,33,3 (terminal 3)
            36,26,6,36,26,6 (terminal 6)
            29,9,29,9,29,9 (terminal 9)
    """
    def __init__(self):
        self.terminais = {i: [n for n in range(37) if n % 10 == i] for i in range(10)}
        self.terminais_alvo = {3, 6, 9}  # Foco nos terminais 3, 6, 9
    ...
    def analisar(self, historico):
        ...
        # Conta frequência dos terminais 3, 6, 9
...
# Defaults com novas estratégias
defaults = {
...
with st.sidebar.expander("🤖 Motores (21)", expanded=False):
    ...
    with c5:
        ...
        st.session_state.usar_micro_clusters = st.checkbox("🔗 Micro-Clusters", value=st.session_state.usar_micro_clusters)
    with c6:
        st.session_state.usar_ritmo_repeticao = st.checkbox("🎵 Ritmo Repetição", value=st.session_state.usar_ritmo_repeticao)
        st.session_state.usar_terminal_369 = st.checkbox("🔢 Terminal 3-6-9", value=st.session_state.usar_terminal_369)
