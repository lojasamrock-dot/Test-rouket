...
# =============================
# 🧠 DUZIA AI V7.1 - 3 NOVAS ESTRATÉGIAS ANTI-ERROS
# =============================
class DuziaAI:
    ...
    def calibrar_confianca(self, confianca, acertou=None):
        ...
        if acertou is not None and not acertou and confianca > 5.0:
            ...
            logging.info(f"📉📉📉 CALIBRAÇÃO IMEDIATA: Confiança {confianca:.2f}")
            return confianca
    ...
    def balancear_previsoes(self, previsao):
        ...
        if self.erros_consecutivos_mesma_duzia >= 2 and self.ultima_duzia_errada:
            ...
            if previsao['duzia'] == dz_bloqueada:
                ...
                logging.info(f"🚫 ANTI-REPETIÇÃO: Bloqueando D{dz_bloqueada}")
                return self._garantir_cobertura_diferente(previsao)
        
        # 🆕 ANTI-INSISTÊNCIA: 2 erros prevendo a MESMA dúzia
        if len(self.ultimas_previsoes) >= 2 and len(self.ultimos_resultados) >= 2:
            if self.ultimas_previsoes[-1] == self.ultimas_previsoes[-2]:
                if not self.ultimos_resultados[-1]['acertou'] and not self.ultimos_resultados[-2]['acertou']:
                    ...
                    # Pega a que tem maior frequência nas últimas 5
        ...
        # MICRO-TENDÊNCIA (score reduzido para 15)
        if len(u) >= 3:
            ultimas_3 = u[-3:]
    ...
    def _eh_caos(self, frequencias):
        if len(frequencias) < 2: return True
        valores = list(frequencias.values())
        return max(valores) <= 3 and max(valores) - min(valores) <= 2
    ...
    def detectar_exaustao_ciclo_dominante(self):
        ...
        if self.sinal_mudanca_pendente:
            if u[-1] == self.sinal_mudanca_pendente['dz_quebra'] and u[-1] != 0:
                resultado = {'tipo': 'MUDANCA_VELOCIDADE', 'dz_quebra': self.sinal_mudanca_pendente['dz_quebra'],
                           'dz_exaurida': self.sinal_mudanca_pendente['dz_exaurida'], 'forca': 9}
        ...
        if freq_10.get(dz_dominante, 0) >= 7:
            ...
            if len(ranking) >= 2:
                ...
                if terceira and sum(1 for d in ultimas_10[-4:] if d == terceira) >= 2:
        ...
        if len(freq) == 2 and freq.most_common(1)[0][1] >= 6:
            ...
            if sum(1 for d in u[-5:] if d == terceira) >= 1:
    ...
    def detectar_terceira_duzia_intrusa(self):
        ...
        if (ranking[0][1] + ranking[1][1]) >= 7 and count_terceira <= 1:
            forca = min(8, 3 + (8 - count_terceira) * 0.7)
    ...
    def detectar_progressao(self):
        ...
        u = list(self.historico)[-4:]
        if u[0] == u[1] and u[2] == u[3] and u[1] != u[2] and u[0] != 0 and u[2] != 0: return u[2], 4
        return None
    
    def detectar_streak_longo(self):
        streak_count, streak_d = self.streak()
        if streak_count >= 3 and streak_d != 0: return streak_d, min(10, 4 + (streak_count - 3) * 2)
        return None
    ...
    def calcular_score(self):
        ...
        if len(self.historico) >= 2:
            ...
            if len(set(ultimas_2)) == 1 and ultimas_2[0] != 0:
                ...
                for d in self._get_outras_duzias(dz_streak):
                    score[d] *= 0.5
        ...
        if len(self.historico) >= 6:
            ...
            if len(ranking_6) >= 2 and ranking_6[0][1] + ranking_6[1][1] >= 5:
                dz1, dz2 = ranking_6[0][0], ranking_6[1][0]
                terceira = self._get_terceira_duzia(dz1, dz2)
                if terceira not in ultimas_6[-4:]:
                    score[terceira] *= 0.2
                    detalhes[terceira].append(f"🚫 FORA DO MOMENTO: -80%")
                    if u[-1] == dz1: score[dz1] += 15; detalhes[dz1].append(f"🎯 MOMENTO: +15")
                    elif u[-1] == dz2: score[dz2] += 15; detalhes[dz2].append(f"🎯 MOMENTO: +15")
        ...
        if len(self.ultimos_resultados) >= 3:
            ...
            if len(set(ultimos_3_real)) == 1 and ultimos_3_acerto.count(False) >= 2:
                ...
                if dz_real != 0:
                    ...
                    for d in self._get_outras_duzias(dz_real): score[d] *= 0.2
        ...
        if exaustao:
            ...
            detalhes[exaustao['dz_quebra']].append(f"🎯 GATILHO: {exaustao.get('descricao','')}")
        ...
        if intrusa:
            dz_terceira, forca, dz1, dz2 = intrusa
            if dz_terceira != 0:
                score[dz_terceira] += self.get_peso_adaptativo('terceira_intrusa', forca * 1.3)
        
        rep = self.detectar_repeticao_imediata()
        if rep: score[rep[0]] += self.get_peso_adaptativo('repeticao_imediata', rep[1])
        
        sl = self.detectar_streak_longo()
        if sl: score[sl[0]] += self.get_peso_adaptativo('streak_longo', sl[1])
        
        vv = self.detectar_vai_e_volta()
        if vv: score[vv[0]] += self.get_peso_adaptativo('vai_e_volta', vv[1])
        
        aba = self.detectar_retorno_aba()
        if aba: score[aba[0]] += self.get_peso_adaptativo('retorno_aba', aba[1])
        
        duas = self.detectar_duas_dominantes()
        if duas:
            for dz in duas[0]:
                if dz != 0: score[dz] += self.get_peso_adaptativo('duas_dominantes', duas[1])
        
        prog = self.detectar_progressao()
        if prog: score[prog[0]] += self.get_peso_adaptativo('progressao', prog[1])
        ...
        if len(self.numeros_completos) >= 10:
            ...
            if v_count > total_janela * 0.45: score[2] += 4; score[3] += 4
            elif t_count > total_janela * 0.40: score[2] += 2; score[3] += 3
            elif o_count > total_janela * 0.35: score[2] += 5
    ...
    def prever(self, confianca_minima=3.2, agressividade=2):
        ...
        if self.alerta_zero_ativo: confianca = confianca * 0.6
            ...
            for det in detalhes[dz]:
                if '+' in det:
                    try:
                        ...
                        if detector_nome not in detectores_ativos: detectores_ativos.append(detector_nome)
                    except: pass
        ...
        if pode_entrar: previsao = self.balancear_previsoes(previsao)
...
class SistemaBot:
    ...
    def processar_novo_numero(self, numero_data):
        if isinstance(numero_data, dict): nr = numero_data['number']
        else: nr = int(numero_data)
        ...
        previsao = self.duzia_ai.prever(st.session_state.get('confianca_minima', 3.2), st.session_state.get('agressividade', 2))
        if previsao['entrar']:
            ...
            self.entrada_ativa = {
                'numeros_apostar': numeros_apostar, 'duzia_prevista': previsao['duzia'],
                'duzia_sec_prevista': previsao.get('duzia_secundaria'),
                'confianca': previsao.get('confianca', 0), 'regime': previsao.get('regime', 'NEUTRO'),
                'score': previsao.get('score', {}), 'detalhes': previsao.get('detalhes', {}),
...
def salvar_resultado_em_arquivo(historico, caminho=HISTORICO_PATH):
    ...
    except Exception as e: logging.error(f"Erro ao salvar histórico: {e}")
...
# APLICAÇÃO STREAMLIT
# =============================
st.set_page_config(page_title="🎰 DuziaAI V7.1 - 3 Novas Estratégias", layout="wide")
st.title("🎰 DuziaAI V7.1 - DETECTOR 2/3 + ANTI-INSISTÊNCIA + ALERTA TERCEIRA (BRT)")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 📊 V7.1 - 3 Novas Estratégias")
    ...
    col1.button("💾 Salvar", on_click=lambda: [salvar_resultado_em_arquivo(st.session_state.historico), salvar_sessao(), st.success("✅")], use_container_width=True)
    col2.button("🗑️ Zerar", on_click=lambda: st.session_state.sistema.zerar() or st.rerun(), use_container_width=True)
    col3.button("📥 CSV", on_click=lambda: exportar_historico_csv(st.session_state.sistema.historico_entradas) and st.success("✅"), use_container_width=True)
...
with col_grafico:
    ...
    if len(sis.historico_numeros) >= 5:
        ...
        if sis.duzia_ai.sinal_mudanca_pendente: st.warning(f"⏳ Sinal Pendente")
        if sis.duzia_ai.streak_ativo: st.info(f"🔄 Streak Ativo: D{sis.duzia_ai.streak_ativo}")
        if sis.duzia_ai.alerta_zero_ativo: st.warning("⚠️ ALERTA ZERO ATIVO!")
        
        if len(sis.historico_numeros) >= 10:
            ultimos_20 = list(sis.historico_numeros)[-20:]
            duzias_hist = [get_duzia(n) for n in ultimos_20]
            fig2 = plt.Figure()
            fig2.add_trace(plt.Scatter(x=list(range(len(duzias_hist))), y=duzias_hist, mode='lines+markers', 
            ...
            if sis.sinais_grafico:
                ...
                for idx, dz in sis.sinais_grafico:
                    pos = idx - offset
                    if 0 <= pos < 20: sinal_x.append(pos); sinal_y.append(dz)
                if sinal_x:
                    fig2.add_trace(plt.Scatter(x=sinal_x, y=sinal_y, mode='markers', name='Sinal',
...
with col_entrada:
    ...
    if sis.duzia_ai.sinal_mudanca_pendente:
        ...
        if incluir_zero: cor = "#FFD700"
        elif gatilho: cor = "#FF6347"
        elif confianca >= 5: cor = "#00CC00"
        elif confianca >= 3.5: cor = "#FFA500"
        else: cor = "#FF4444"
        
        limite = "1-12" if duzia_prevista == 1 else "13-24" if duzia_prevista == 2 else "25-36" if duzia_prevista == 3 else "?"
        
        if incluir_zero:
            st.markdown(f"""<div style="background-color:#FFD70022;border:2px solid #FFD700;border-radius:15px;padding:20px;margin:10px 0;">
                <h2 style="color:#FFD700;text-align:center;">⚠️ ALERTA ZERO!</h2>
                <h3 style="text-align:center;">D{duzia_prevista} ({limite}) + 🟢 ZERO</h3>
                <p style="text-align:center;">Confiança: {confianca:.2f} | {regime}</p></div>""", unsafe_allow_html=True)
        elif gatilho:
            st.markdown(f"""<div style="background-color:#FF634722;border:2px solid #FF6347;border-radius:15px;padding:20px;margin:10px 0;">
                <h2 style="color:#FF6347;text-align:center;">🎯 GATILHO!</h2>
                <h3 style="text-align:center;">D{duzia_prevista} ({limite})</h3>
                <p style="text-align:center;">Confiança: {confianca:.2f} | {regime}</p></div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style="background-color:{cor}22;border:2px solid {cor};border-radius:15px;padding:20px;margin:10px 0;">
                <h2 style="color:{cor};text-align:center;">🎯 D{duzia_prevista} ({limite})</h2>
                <p style="text-align:center;">Confiança: {confianca:.2f} | {regime}</p></div>""", unsafe_allow_html=True)
        
        if detectores: st.caption(f"🎯 Detectores: {', '.join(detectores)}")
        if numeros_apostar:
            st.write("**🎲 Números:**")
            cols = st.columns(7)
            for i, n in enumerate(sorted(numeros_apostar)):
                if n == 0: cols[i % 7].button("🟢 0", key="num_zero", use_container_width=True)
                else: cols[i % 7].button(str(n), key=f"num_{n}", use_container_width=True)
        st.progress(min(1.0, max(0.0, confianca / 10.0)))
    else:
        st.info("🔍 Analisando padrões...")
    if sis.ultimo_numero is not None:
        st.markdown("---")
        if sis.ultimo_numero == 0: st.write("**🔄 Último Giro:** 🟢 ZERO")
        else: st.write(f"**🔄 Último Giro:** #{sis.ultimo_numero} → D{get_duzia(sis.ultimo_numero)}")
...
if sis.historico_entradas:
    ...
    for e in reversed(sis.historico_entradas[-20:]):
        ...
        dados_tabela.append({"Rodada": e.get('rodada', '-'), "Hora (BRT)": e.get('hora', '-'),
                           "Número": e.get('numero', '-'), "Dúzia Real": duzia_real_str,
                           "Dúzia Prevista": f"D{e.get('duzia_prevista', '?')}" if e.get('duzia_prevista') else "-",
                           "Confiança": f"{e.get('confianca', 0):.2f}", "Gatilho": gatilho_str, "Resultado": resultado_str})
...
st.markdown("---")
st.caption(f"🤖 DuziaAI V7.1 | 3 Novas Estratégias Anti-Erros + BRT | {formatar_hora_brasilia()}")
salvar_sessao()
