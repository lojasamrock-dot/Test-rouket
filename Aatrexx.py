...
```
    ...
    session_data = {
        ...
        'confianca_minima': st.session_state.get('confianca_minima', 3.5),
...
# 🧠 META-MODELO V8.5.2 - COMITÊ DE ESPECIALISTAS
...
def especialista_tendencia(self, duzia_ai, previsao):
    ...
    # Streak forte a favor
    if streak_count >= 2 and streak_d == previsao['duzia']:
        score += 2
    
    # Frequência > 35% nos últimos 10
...
def especialista_qualidade(self, duzia_ai, previsao):
    ...
    V9.0 - Mais rigoroso: exige qualidade >= 45, penaliza erros consecutivos.
    """
    score = 0
    qualidade = duzia_ai._calcular_qualidade_entrada(previsao)
    mercado_ruidoso = duzia_ai._mercado_ruidoso()
    
    # Qualidade mínima - limiar elevado para V9
    if qualidade >= 55:
        score += 3
    elif qualidade >= 45:
        score += 2
    ...
    # Sem ruído - peso maior
    if not mercado_ruidoso:
        score += 2
    else:
        score -= 1
    
    # Confiança razoável - limiar mais alto
    ...
    # Erros recentes - mais penalizador
...
def especialista_confirmacao(self, duzia_ai, previsao):
    """
    V9.0 - NOVO ESPECIALISTA: Confirmação multi-janela.
    Valida se a duzia prevista tem consistência em múltiplas janelas temporais.
    """
    score = 0
    duzia = previsao.get('duzia', 0)
    if duzia == 0:
        return False, 0
    
    hist = list(duzia_ai.historico)
    
    # Janela curta (últimos 5): a duzia deve aparecer
    if len(hist) >= 5:
        freq_5 = Counter([d for d in hist[-5:] if d != 0])
        total_5 = sum(freq_5.values())
        if total_5 > 0:
            pct_5 = freq_5.get(duzia, 0) / total_5
            if pct_5 >= 0.4:
                score += 3
            elif pct_5 >= 0.25:
                score += 1
    ...
    # Janela média (últimos 10)
    if len(hist) >= 10:
        freq_10 = Counter([d for d in hist[-10:] if d != 0])
        total_10 = sum(freq_10.values())
        if total_10 > 0:
            pct_10 = freq_10.get(duzia, 0) / total_10
            if pct_10 >= 0.35:
                score += 2
            elif pct_10 >= 0.20:
                score += 1
    
    # Última entrada foi acerto?
    if duzia_ai.ultimos_resultados:
        ultimo = duzia_ai.ultimos_resultados[-1]
        if ultimo.get('acertou', False):
            score += 1
        else:
            score -= 1
    
    # Sem inconsistência contra mudança brusca
    mudanca = duzia_ai.detectar_mudanca_abrupta()
    if mudanca:
        dz_nova, _, _ = mudanca
        if dz_nova != duzia:
            score -= 2
...
def votar(self, duzia_ai, previsao):
    """
    V9.0 - Comitê de 4 especialistas. Precisa de 3+ aprovações para entrar.
    Mais restritivo = menos entradas, mas de maior qualidade.
    """
    votos = []
    scores = {}
    ...
    aprovado_cf, score_cf = self.especialista_confirmacao(duzia_ai, previsao)
    votos.append(('Confirmação', aprovado_cf))
    scores['Confirmação'] = score_cf
    
    aprovacoes = sum(1 for _, v in votos if v)
    
    # V9: exige 3 de 4 especialistas (mais rigoroso que V8 que exigia 2/3)
    aprovado_final = aprovacoes >= 3
    ...
    return aprovado_final, votos, scores, votos_str
...
# 🧠 DUZIA AI V8.5.2 - CORRIGIDO (PROTEÇÃO CONTRA ZERO)

# =============================

class DuziaAI:
def **init**(self, window=30):
...
def _mercado_ruidoso(self):
    ...
    taxa = trocas / 9
    return taxa > 0.70  # V9: detecta ruído mais cedo que V8 (era 0.75)
...
def calibrar_confianca(self, confianca, acertou=None):
    ...
    # V9.0: Penalidade progressiva mais agressiva por erros seguidos
    if len(self.historico_confianca) >= 2:
        ultimos = self.historico_confianca[-5:]
        erros_seguidos = 0
        for h in reversed(ultimos):
            if not h.get('acertou', True):
                erros_seguidos += 1
            else:
                break
        ...
        if erros_seguidos >= 4:
            confianca *= 0.15
            logging.info(f"📉📉📉📉 4 ERROS SEGUIDOS: Confiança reduzida 85%")
        elif erros_seguidos >= 3:
            confianca *= 0.25
            logging.info(f"📉📉📉 3 ERROS SEGUIDOS: Confiança reduzida 75%")
        elif erros_seguidos >= 2:
            confianca *= 0.45
            logging.info(f"📉📉 2 ERROS SEGUIDOS: Confiança reduzida 55%")
...
# =============================================
# 🎯 GATILHOS (COMPLETO V7.0 + PROTEÇÃO ZERO)
# =============================================
...
# ========== DETECTORES UNIVERSAIS (COMPLETO V7.0 + PROTEÇÃO ZERO) ==========
...
# ========== CÁLCULO DE SCORE PRINCIPAL (V8.5.2 - PROTEÇÃO ZERO) ==========
...
def calcular_score(self):
    ...
    # V9.0 - Detector Janela Dupla: duzia domina tanto nos últimos 5 quanto nos últimos 10
    if len(self.historico) >= 10:
        u = list(self.historico)
        freq_5 = Counter([d for d in u[-5:] if d != 0])
        freq_10 = Counter([d for d in u[-10:] if d != 0])
        total_5 = sum(freq_5.values())
        total_10 = sum(freq_10.values())
        if total_5 > 0 and total_10 > 0:
            for dz in [1, 2, 3]:
                pct_5 = freq_5.get(dz, 0) / total_5
                pct_10 = freq_10.get(dz, 0) / total_10
                if pct_5 >= 0.40 and pct_10 >= 0.35 and dz != 0:
                    bonus_dupla = 12
                    score[dz] += bonus_dupla
                    detalhes[dz].append(f"🔥 JANELA DUPLA D{dz} ({pct_5*100:.0f}%/5 + {pct_10*100:.0f}%/10): +{bonus_dupla}")
                    break
...
def prever(self, confianca_minima=3.2, agressividade=2):
    ...
    score_muito_forte = s1 > 50 and s1 > s2 * 1.6  # V9: limiar mais alto que V8 (45/1.5)
    ...
    # Qualidade mínima - V9: limiar elevado para 50
    if pode_entrar and not tem_gatilho_quebra and not score_muito_forte:
        qualidade = self._calcular_qualidade_entrada(previsao)
        if qualidade < 50 and not tem_consenso:
            pode_entrar = False
            previsao['entrar'] = False
            previsao['motivo'] = f"📊 Qualidade insuficiente ({qualidade:.0f}/100 < 50)"
            logging.info(f"🚫 Entrada bloqueada por qualidade: {qualidade:.0f}/100")
    
    # V9.0: Bloqueio de pausa após 2+ erros consecutivos com confiança baixa
    if pode_entrar:
        erros_seq = 0
        for r in reversed(self.ultimos_resultados[-4:]):
            if not r.get('acertou', True):
                erros_seq += 1
            else:
                break
        if erros_seq >= 2 and confianca < 4.0:
            pode_entrar = False
            previsao['entrar'] = False
            previsao['motivo'] = f"⏸️ Pausa após {erros_seq} erros consecutivos (confiança {confianca:.2f} < 4.0)"
            logging.info(f"⏸️ V9: Pausa por {erros_seq} erros consecutivos")
...
st.set_page_config(page_title=“🎰 DuziaAI V9.0 - Michele Edition”, layout=“wide”)
st.title(“🎰 DuziaAI V8.5.2 - PROTEÇÃO ZERO CORRIGIDA (BRT)”)
...
st.markdown("---")
st.markdown("### 🛡️ V8.5.2 - PROTEÇÃO ZERO")
st.caption("🔧 KeyError com ZERO resolvido")
st.caption("🛡️ Proteção em TODOS os acessos")
st.caption("✅ score[d] e detalhes[d] validados")
st.caption("🎯 Gatilhos com filtro dz != 0")
st.caption("✅ Base V8.5.1 mantida")
...
# V9: Colorir Win Rate conforme meta de assertividade

win_delta = f”{‘▲ acima meta’ if tx_acerto >= 87 else ‘▼ abaixo meta’}” if total_entradas >= 5 else None
col3.metric(“📊 Win Rate”, f”{tx_acerto:.1f}%”, delta=win_delta)
...
if len(sis.historico_numeros) >= 5:
    ...
    if sis.duzia_ai.meta_modelo.ultima_decisao:
        with st.expander("🧠 Última Votação Meta-Modelo (4 Esp.)", expanded=False):
            decisao = sis.duzia_ai.meta_modelo.ultima_decisao
            aprovacoes = decisao.get('aprovacoes', 0)
            st.caption(f"**Aprovações: {aprovacoes}/4** {'✅ ENTRADA' if decisao.get('aprovado') else '❌ BLOQUEADO'}")
            for nome, voto in decisao['votos']:
                icon = "✅" if voto else "❌"
                score_v = decisao['scores'].get(nome, 0)
...
st.markdown(”—”)
st.caption(f”🤖 DuziaAI V9.0 Michele Edition | Alta Assertividade | {formatar_hora_brasilia()}”)
salvar_sessao()
